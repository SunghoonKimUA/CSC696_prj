...
performance_dist = []
performance_epochs = 5

import itertools
from more_itertools import random_product, sample
...
ecoinds_cnt = 20
ecoinds_arr = np.ndarray(shape=(ecoinds_cnt+1, 10), dtype=int)
for e_idx in range(ecoinds_cnt):
    ecoinds_arr[e_idx] = sample(range(19), k=10)
ecoinds_arr[ecoinds_cnt] = np.zeros(shape=(10))
...
for p_idx in range(performance_epochs):
    ...
    S = shuffle(S)
    # print(S.shape)
    print("Start Time of iteration("+str(p_idx)+") =", datetime.now().strftime("%H:%M:%S"))
    ...
    idx = 'BaseLine'
    performance_dist.append([idx, '', '', p_idx, 0.0, base_acc, base_mse])

    X_train_arr = X_train.reshape((X_train.shape[0], X_train.shape[1], 1))
    X_test_arr  = X_test.reshape((X_test.shape[0], X_test.shape[1], 1))
    idx = 'grades_only'
    test_loss, test_acc, test_mse = training_model(X_train_arr, y_train, X_test_arr, y_test, idx, 'rnn')
    print("★★★Grades only's Accuracy:",test_acc)

    performance_dist.append([idx, '', 'rnn', p_idx, test_loss, test_acc, test_mse])
    ...
    for var1_idx, var2_idx, var3_idx, read_mode, num_of_evnt_cd, bool_indicators in experiment_arr:
        ...
        for tmp_ecoinds in ecoinds_arr:
            print(tmp_ecoinds)
            ...
            for dim1_idx in range(org_X.shape[0]):
                ...
                for dim2_idx in range(history_len):
                    ...
                    if bool_indicators == 1:
                        X[dim1_idx,dim2_idx,col_idx] = get_indicator_score(normalized_indic, org_X[dim1_idx, dim2_idx+history_len+1], org_X[dim1_idx, 0], tmp_ecoinds)
                        # if dim1_idx < 10:
                        #     print(X[dim1_idx,dim2_idx,col_idx])
                        col_idx = col_idx + 1
            ...
            performance_dist.append([idx, tmp_ecoinds, archi, p_idx, test_loss, test_acc, test_mse])
...
pd.DataFrame(performance_dist, columns=['input_option', 'economic_inds', 'archi', 'iter_no', 'loss', 'accuracy', 'mean squared error']).to_csv("./data/choose_ecoinds_from_normzed_cnt.csv")
