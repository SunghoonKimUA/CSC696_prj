...
def training_model(X_train_arr, y_train, X_test_arr, y_test, idx, archi):
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            "./model/final_"+idx+"_model.h5", save_best_only=True, monitor="val_loss"
        ),
    ...
    model = keras.models.load_model("./model/final_"+idx+"_model.h5")
...
#   ['ev', 'ts'],
   ['ev'],
   ['nm'],#'ex', 
   ['10', '15', '20', '30'],#date gap between news and eval'0', '5', '10', '15', '20', '30'
   ['1', '2'],#read_mode(subj vs subj+obj), '2'
   [5, 10],# number of event_code, 5, 10, 15
   [0, 1, 2, 3, 4],#include economic indicators or not, 0, 1, 2, 3, 4
   # 0: non indicators
   # 1: all indicators
   # 2: [ 0  1 16 14  4 17  6  7 12 10]
   # 3: [ 0 14  2 17 18  5 13 11  8 15]
   # 4: [15 10  2  3  4  5  6  7  8 17]
   ['cnt','tone']
]
...
timeseries_cand = [36]#60, 36, 12, 6, 3
archi_cand = ['lstm']
norm_prd_cnt = 10
...
for history_len in timeseries_cand:
    for p_idx in range(performance_epochs):
        ...
        # experiment_arr = get_random_option_lists(option_lists, 10)
        ...
        idx = 'BaseLine'
        performance_dist.append([idx, 0, '', p_idx, 0.0, base_acc, base_mse])

        X_train_arr = X_train.reshape((X_train.shape[0], X_train.shape[1], 1))
        X_test_arr  = X_test.reshape((X_test.shape[0], X_test.shape[1], 1))
        idx = 'grades_only'
        test_loss, test_acc, test_mse = training_model(X_train_arr, y_train, X_test_arr, y_test, idx, 'lstm')
        print("★★★Grades only's Accuracy:",test_acc)

        performance_dist.append([idx, 0, '', p_idx, test_loss, test_acc, test_mse])
        ...
        for var1_idx, var2_idx, var3_idx, read_mode, num_of_evnt_cd, bool_indicators, tp_score in experiment_arr:
            ...
            if bool_indicators == 1 or bool_indicators == 2 or bool_indicators == 3 or bool_indicators == 4:
            ...
            for dim1_idx in range(org_X.shape[0]):
                ...
                for dim2_idx in range(history_len):
                    ...
                    if bool_indicators == 1:
                        ...
                        col_idx = col_idx + 1
                    elif bool_indicators == 2:
                        X[dim1_idx,dim2_idx,col_idx] = get_indicator_score(normalized_indic, org_X[dim1_idx, dim2_idx+history_len+1], org_X[dim1_idx, 0], [ 0,  1, 16, 14,  4, 17,  6,  7, 12, 10])
                        col_idx = col_idx + 1
                    elif bool_indicators == 3:
                        X[dim1_idx,dim2_idx,col_idx] = get_indicator_score(normalized_indic, org_X[dim1_idx, dim2_idx+history_len+1], org_X[dim1_idx, 0], [ 0, 14,  2, 17, 18,  5, 13, 11,  8, 15])
                        col_idx = col_idx + 1
                    elif bool_indicators == 4:
                        X[dim1_idx,dim2_idx,col_idx] = get_indicator_score(normalized_indic, org_X[dim1_idx, dim2_idx+history_len+1], org_X[dim1_idx, 0], [15, 10,  2,  3,  4,  5,  6,  7,  8, 17])
                        col_idx = col_idx + 1
            ...
            for archi in archi_cand:
                idx = build_idx(var1_idx, var2_idx, var3_idx, read_mode, num_of_evnt_cd, bool_indicators)+"_"+tp_score+"__"+str(history_len)
                test_loss, test_acc, test_mse = training_model(X_train, y_train, X_test, y_test, idx, archi)
                performance_dist.append([idx, bool_indicators, tp_score, p_idx, test_loss, test_acc, test_mse])
...
pd.DataFrame(performance_dist, columns=['input_option', 'indicators', 'type_of_score', 'iter_no', 'loss', 'accuracy', 'mean squared error']).to_csv("./data/final_performance_0505.csv")
