...
def training_model(X_train_arr, y_train, X_test_arr, y_test, idx, archi):
    ...
    callbacks = [
        keras.callbacks.ModelCheckpoint(
            "./model/best_timeseries1_"+idx+"_model.h5", save_best_only=True, monitor="val_loss"
        ),
    ...
    model = keras.models.load_model("./model/best_timeseries1_"+idx+"_model.h5")
...
#   ['ev', 'ts'],
   ['ev'],
   ['nm'],#'ex', 
   ['15'],#date gap between news and eval'0', '5', '10', '15', '20', '30'
   ['1'],#read_mode(subj vs subj+obj), '2'
   [10],# number of event_code, 5, 10, 15
   [1],#include economic indicators or not, 0, 1
]
...
timeseries_cand = [12, 6, 3]#60, 36, 12, 6, 3
...
for history_len in timeseries_cand:
    for p_idx in range(performance_epochs):
        ...
        X_test_baseline = X_test[:,-1].reshape((X_test[:,-1].shape[0],1))
        ...
        performance_dist.append([idx, history_len, 'rnn', p_idx, test_loss, test_acc, test_mse])
        ...
        for var1_idx, var2_idx, var3_idx, read_mode, num_of_evnt_cd, bool_indicators in experiment_arr:
            ...
            # print(gdelt_idx+"//"+str(get_avg_gdelt_score(Sum, m_up_feat_1, 86, 'RUS', gdelt_idx+"_up", '1')))
            # print(gdelt_idx+"//"+str(get_avg_gdelt_score(Sum, m_up_feat_1, 87, 'RUS', gdelt_idx+"_up", '1')))
            # print(gdelt_idx+"//"+str(get_avg_gdelt_score(Sum, m_up_feat_1, 88, 'RUS', gdelt_idx+"_up", '1')))
            # print(gdelt_idx+"//"+str(get_avg_gdelt_score(Sum, m_down_feat_1, 86, 'RUS', gdelt_idx+"_down", '1')))
            # print(gdelt_idx+"//"+str(get_avg_gdelt_score(Sum, m_down_feat_1, 87, 'RUS', gdelt_idx+"_down", '1')))
            # print(gdelt_idx+"//"+str(get_avg_gdelt_score(Sum, m_down_feat_1, 88, 'RUS', gdelt_idx+"_down", '1')))
            for dim1_idx in range(org_X.shape[0]):
                ...
                for dim2_idx in range(history_len):
                    ...
                    X[dim1_idx,dim2_idx,col_idx] = max(0.0,get_avg_gdelt_score(Sum, m_up_feat_1, date_range_lst.get_loc(org_X[dim1_idx, dim2_idx+history_len+1]), org_X[dim1_idx, 0], gdelt_idx+"_up", '1'))
                    col_idx = col_idx + 1
                    X[dim1_idx,dim2_idx,col_idx] = min(0.0,get_avg_gdelt_score(Sum, m_down_feat_1, date_range_lst.get_loc(org_X[dim1_idx, dim2_idx+history_len+1]), org_X[dim1_idx, 0], gdelt_idx+"_down", '1'))
                    col_idx = col_idx + 1
                    # next column : get_avg_gdelt_score which read_mode = 2
                    if read_mode == '2':
                        X[dim1_idx,dim2_idx,col_idx] = max(0.0,get_avg_gdelt_score(Sum1, m_up_feat_2, date_range_lst.get_loc(org_X[dim1_idx, dim2_idx+history_len+1]), org_X[dim1_idx, 0], gdelt_idx+"_up", '2'))
                        col_idx = col_idx + 1
                        X[dim1_idx,dim2_idx,col_idx] = min(0.0,get_avg_gdelt_score(Sum1, m_down_feat_2, date_range_lst.get_loc(org_X[dim1_idx, dim2_idx+history_len+1]), org_X[dim1_idx, 0], gdelt_idx+"_down", '2'))
                        col_idx = col_idx + 1
...
pd.DataFrame(performance_dist, columns=['input_option', 'history_len', 'archi', 'iter_no', 'loss', 'accuracy', 'mean squared error']).to_csv("./data/choose_his_len1.csv")
